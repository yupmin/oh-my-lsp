...
const describeIfJSONServer = hasCommand("biome") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("json")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.json"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.json"),
  }
}
...
describeIfJSONServer("CLI integration (JSON)", () => {
  it("runs symbols", () => {
