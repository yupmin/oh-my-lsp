...
const describeIfSwiftServer = hasCommand("sourcekit-lsp") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("swift")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.swift"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.swift"),
  }
}
...
describeIfSwiftServer("CLI integration (Swift)", () => {
  it("runs symbols", () => {
