...
const describeIfNixServer = hasCommand("nixd") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("nix")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.nix"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.nix"),
  }
}
...
describeIfNixServer("CLI integration (Nix)", () => {
  it("runs symbols", () => {
