...
const describeIfTSXServer = hasCommand("typescript-language-server") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("tsx")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.tsx"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.tsx"),
  }
}
...
describeIfTSXServer("CLI integration (TSX)", () => {
  it("runs symbols", () => {
