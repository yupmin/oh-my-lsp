...
const describeIfHaskellServer = hasCommand("haskell-language-server-wrapper") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("haskell")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.hs"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.hs"),
  }
}
...
describeIfHaskellServer("CLI integration (Haskell)", () => {
  it("runs symbols", () => {
