...
const describeIfYamlServer = hasCommand("yaml-language-server") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = track(createFixtureWorkspace("yaml"))
  return {
    workspace,
    sampleFile: join(workspace, "sample.yaml"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.yaml"),
  }
}

describeIfYamlServer("CLI integration (YAML)", () => {
  it("runs symbols", () => {
