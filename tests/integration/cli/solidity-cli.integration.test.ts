...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = createFixtureWorkspace("solidity")
  workspaces.push(workspace)
  return {
    workspace,
    sampleFile: join(workspace, "sample.sol"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.sol"),
  }
}
...
function expectNotConfigured(result: { error?: Error; status: number | null; stdout: string }): void {
  ...
  expect(result.stdout).toContain("No LSP server configured for extension: .sol")
}

describe("CLI integration (Solidity)", () => {
