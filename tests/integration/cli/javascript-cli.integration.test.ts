...
const describeIfJavascriptServer = hasCommand("typescript-language-server") ? describe : describe.skip
...
function createWorkspaceFiles(): { workspace: string; sampleFile: string; diagnosticsFailFile: string } {
  const workspace = track(createFixtureWorkspace("javascript"))
  return {
    workspace,
    sampleFile: join(workspace, "sample.js"),
    diagnosticsFailFile: join(workspace, "diagnostics_fail.js"),
  }
}

describeIfJavascriptServer("CLI integration (JavaScript)", () => {
  it("runs symbols", () => {
